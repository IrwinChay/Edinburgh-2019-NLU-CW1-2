...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            1.Get the output of the model by giving the parameters, tokens of source sentence, length of source sentence 
            and the target input.
            2.Calculate the loss by using the output of the model and the target sentence, and normalize it with sthe length 
            of source sentence.
            3.Call the backward function to calucluate the gradient through the back propogation.
            4. The norm is obtained by calling the function clip_grad_norm_ which is regard as the single vector, 
            and the gradients are updated in-place.
            5. Call the step() to update parameters(weight) of the neural network model.
            6. Clear the gradients for the further update.
