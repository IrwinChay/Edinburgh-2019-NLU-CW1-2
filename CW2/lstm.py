...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        The combine_directions function is combining results of bidirectional propogation.
        It use cat function to combine odd position (from left to right) of the output and 
        even position(from right to left) of the output.
        In the bidirectional recurrent neural network, final_hidden_state and
         final_cell_state will be combination of reuslts of birectional propogation.
        The output layer of bidirectional recurrent neural network will get information from past state and future state.
        The final_hidden_states is the final output of hidden layers which is used  to pass the information.
        The final_cell_states is the final output of the memory cell which is used to keep the long-distance information.
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        1. Get the variable-length alignment vevtor(attn_weights) by using function softmax and the parameters attention score.
        2. The attention context vector is calculated by the  matrix-matrix product of the alignment vector and the output of the encoder.
        Pytorch could deal with Tesor. So source sentences will be padded. 
        The padded position will be applied a mask in order to guarantee correct attention score for these positions in the source sentences.
    ...
    def score(self, tgt_input, encoder_out):
        ...
        The attention socre is obtained by the matrix multiplication of the transpose of all target hidden states and all source hidden states.
        1.To concatenate target hidden state vector and source hidden state vector, the maxtrix multiplication can adjust the dimension of the vector in each time step.
        2. The matrix multiplication of target hidden state matrix and source hidden state matrix is the attentional score. 
        The attentional is applied to the sofmax function to get global align weights.
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            self.lexical_model_layer = nn.Linear(embed_dim, embed_dim, bias=False)
            self.combine_lexical_and_hidden_layer = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    current_context = torch.tanh(torch.bmm(step_attn_weights.unsqueeze(dim=1),src_embeddings))
                    src_embeddings = src_embeddings.transpose(0,1)
        ...
        if self.use_lexical_model:
            ...
            lexical_model_output = torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim)
            lexical_model_layer_output =torch.tanh(self.lexical_model_layer(lexical_model_output))+lexical_model_output
            combing_lexical_model_layer = self.combine_lexical_and_hidden_layer(lexical_model_layer_output)
            decoder_output = decoder_output + combing_lexical_model_layer.transpose(0,1)
            # TODO: --------------------------------------------------------------------- /CUT
