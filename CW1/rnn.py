...
class RNN(object):
    ...
    def __init__(self, vocab_size, hidden_dims, out_vocab_size):
        ...
        self.W = np.random.randn(
            self.out_vocab_size, self.hidden_dims) * np.sqrt(0.1)
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            ...
            # --- your code here --- #
            ##########################
            x_onehot_t = make_onehot(x[t], self.vocab_size)
            net_in_t = np.add(np.dot(self.V, x_onehot_t), np.dot(self.U, s[t - 1]))
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            delta_out_t = d_onehot - y[t]
            #delta_out_t = np.multiply(d_onehot - y[t],np.ones(self.out_vocab_size))
            self.deltaW += np.outer(delta_out_t, s[t])
            delta_in_t = np.dot(self.W.T, delta_out_t) * grad(s[t])
            x_onehot = make_onehot(x[t], self.out_vocab_size)
            self.deltaV += np.outer(delta_in_t, x_onehot)
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in reversed(range(len(x))):
            ...
            for i in range(t-1, t-steps-1, -1):
                if i >= 0:
                    delta_in_t_n = np.dot(self.U.T, delta_in_t) * grad(s[i])
                    delta_in_t = delta_in_t_n
                    x_onehot = make_onehot(x[i], self.out_vocab_size)
                    self.deltaV += np.outer(delta_in_t_n, x_onehot)
                    self.deltaU += np.outer(delta_in_t_n, s[i-1])
    ...
    def acc_deltas_bptt_np(self, x, d, y, s, steps):
        ...
        # --- your code here --- #
        ##########################
        #d_onehot = make_onehot(d, self.out_vocab_size)
        delta_out_t = d - y[-1]
        #delta_out_t = np.multiply(d_onehot - y[t],np.ones(self.out_vocab_size))
        self.deltaW += np.outer(delta_out_t, s[-1])
        delta_in_t = np.dot(self.W.T, delta_out_t) * grad(s[-1])
        x_onehot = make_onehot(x[-1], self.out_vocab_size)
        self.deltaV += np.outer(delta_in_t, x_onehot)
        self.deltaU += np.outer(delta_in_t, s[t - 1])
    ...
    def compute_loss(self, x, d):
        ...
        ##########################
        y, s = self.predict(x)
        # d_onehot = np.zeros((len(x), self.out_vocab_size))
        for t in range(len(x)):
            # d_onehot[:,t] = make_onehot(d[t])
            d_onehot_t = make_onehot(d[t], self.out_vocab_size)
            loss -= np.dot(d_onehot_t, np.log(y[t]))
        # loss = loss / len(x) #compute the loss for the sentence average on the step
    ...
    def compute_acc_lmnp(self, X_dev, D_dev):
        ...
        '''
        acc = sum([self.compare_num_pred(X_dev[i], D_dev[i])
    ...
    def compute_mean_loss(self, X, D):
        ...
        ##########################
        for t in range(len(X)):
            t_loss = self.compute_loss(X[t], D[t])                                                                                                                                                                                                                                                                                                                                                                                                                       
            size += len(D[t])
            mean_loss += t_loss
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        # --- your code here --- #
        ##########################
        nn = RNN(vocab_size, hdim, vocab_size)
        b_loss = nn.train(X_train, D_train, X_dev, D_dev, epochs = 10, back_steps = lookback)

        np.save(data_folder + '/rnn.U.npy', nn.U)
        np.save(data_folder + '/rnn.V.npy', nn.V)
        np.save(data_folder + '/rnn.W.npy', nn.W)

        #print(b_lost)
        run_loss = nn.compute_mean_loss(X_dev, D_dev)
        frac_loss = fraq_loss(vocab, word_to_num, vocab_size)
        adjusted_loss = adjust_loss(run_loss, frac_loss, q)
